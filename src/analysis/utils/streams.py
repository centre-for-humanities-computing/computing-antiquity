...
@reusable
def sentence_stream(files: List[str], normalize: Callable, lemmatize: Callable):
    for file in files:
        with open(file) as f:
            text = f.read()
        text = normalize(text)
...
@reusable
def stream_files(paths : Iterable[str], tolerate_failure: bool = True) -> Iterable[str]:
    """
...
@reusable
def progress_bar_stream(items: Iterable[I], total: Optional[int] = None) -> Iterable[I]:
    """
